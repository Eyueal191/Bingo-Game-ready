...
const initializeSpinSocket = (io) => {
  io.on("connection", (socket) => {
    logger.info(`Spin socket connected: ${socket.id}`);
    ...
    const eventNameFor = (gameType, suffix) => {
      if (!gameType) gameType = "fetan-spin";
    ...
    const finalizeIfWinnersPresent = async (gameDoc) => {
      try {
        ...
        logger.warn(
          `[SpinSocket] Reconciled payouts and completed stuck game ${freshGame._id}`
        );
      } catch (e) {
        logger.error(
    ...
    socket.on("requestSpinData", async () => {
      try {
        ...
        logger.debug("Games for requestSpinData fetched", {
          socketId: socket.id,
        ...
        const gamesWithPrizeStructure = games
          ...
          .map((game) => {
            ...
            return {
              ...
              prize_tiers: tiers,
              gameType: game.gameType || "fetan-spin",
            };
          });

        // Emit legacy fetan-spin list and per-gameType lists
        const keshList = gamesWithPrizeStructure.filter(
          (g) => (g.gameType || "fetan-spin") === "fetan-spin"
        );
        socket.emit("fetan_spin_rooms", keshList); // Always emit, even if empty
        const others = gamesWithPrizeStructure.filter(
          (g) => (g.gameType || "fetan-spin") !== "fetan-spin"
        );
        // group by gameType
        const grouped = others.reduce((acc, g) => {
          const key = (g.gameType || "fetan-spin").replace(/-/g, "_");
          acc[key] = acc[key] || [];
          acc[key].push(g);
          return acc;
        }, {});
        ...
        logger.error(`Error fetching Spin game rooms: ${error.message}`);
        socket.emit("error", {
          message: "Failed to load Spin game rooms",
        });
      }
    });
      ...
      async ({ stakeAmount, userId, maxPlayers }) => {
        try {
          ...
          if (!game) {
            ...
            logger.info(
              `Created new Spin game ${game._id} for stake ${stakeAmount}, max_players ${max_players}`
            );
          }
          ...
          logger.debug("Games for spin_get_room_by_stake fetched", {
            socketId: socket.id,
            count: Array.isArray(games) ? games.length : 0,
          });
          ...
          // Emit grouped rooms per type including legacy fetan-spin
          const keshRooms = gamesWithPrizeStructure.filter(
            (g) => (g.gameType || "fetan-spin") === "fetan-spin"
          );
          if (keshRooms.length) io.emit("fetan_spin_rooms", keshRooms);
          ...
          logger.error(
            `Error in Spin get_room_by_stake: ${error.message}`
          );
          socket.emit("error", {
            message: "Failed to fetch or create Spin room",
          });
        }
      }
    );
      ...
      async ({ stakeAmount, maxPlayers }) => {
        try {
          ...
          socket.emit("active_games_by_stake", {
            ...
            gameType: activeGames.length ? activeGames[0].gameType : "fetan-spin",
          });
        } catch (error) {
          logger.error(
            `Error in Spin get_active_games_by_stake: ${error.message}`
          );
          socket.emit("error", {
            message: "Failed to fetch active Spin games",
          });
        }
      }
    );
    ...
    socket.on("join_spin", async (gameId) => {
      let game;
      try {
        if (!gameId) {
        ...
        logger.debug("join_spin loaded game", {
          socketId: socket.id,
          gameId,
          found: Boolean(game),
          status: game?.status,
        ...
        socket.join(gameId);
        logger.info("Socket joined Spin room", {
          socketId: socket.id,
          gameId,
        });
        ...
        logger.error(
          `Error joining Spin game ${game - gameId}: ${error.message}`
        );
        socket.emit("error", {
          message: "Failed to join Spin game",
          gameId,
        });
      }
    });
    ...
    socket.on(
      ...
      async ({ gameId, userId, selectedNumbers }) => {
        try {
          ...
          if (!game) {
            socket.emit("error", {
              message: "Spin game not available",
              gameId,
            });
            return;
          }
          ...
          logger.debug("Updated game for join_spin_game", {
            socketId: socket.id,
            gameId,
          ...
          const roomData = {
            ...
            prize_tiers: tiers,
            gameType: updatedGame.gameType || "fetan-spin",
          };
          ...
          if (user) {
            ...
            try {
              ...
              logger.warn("Failed to send Spin purchase confirmation", {
          ...
          if (
            ...
            logger.debug("Spin game moved to in_progress", {
              gameId,
            ...
            io.to(gameId).emit("gameUpdate", {
              type: "status",
              status: "in_progress",
              message: `${refreshedGame.gameType || "Spin"
                } game starting...`,
              gameType: refreshedGame.gameType || "fetan-spin",
              roomId: String(gameId),
            });
            ...
            setTimeout(async () => {
              ...
              let segments = null;
              if ((refreshedGame2.gameType || "fetan-spin") === "fetan-spin") {
              ...
              for (const t of sortedTiers) {
                ...
                if (part) {
                  ...
                  try {
                    ...
                    logger.error(`Failed to execute fetan-spin payout transaction: ${e.message}`);
                  } finally {
                    session.endSession();
                  }
              ...
              setTimeout(async () => {
                ...
                io.to(gameId).emit("gameUpdate", {
                  type: "status",
                  status: "completed",
                  gameType: afterWinnersGame.gameType || "fetan-spin",
                  roomId: String(gameId),
                });
                ...
                try {
                  ...
                  const newGame = await Game.create({
                    ...
                    status: "pending",
                    gameType: oldGame.gameType || "fetan-spin",
                  ...
                  io.to(gameId).emit(
                    eventNameFor(newGame.gameType, "room_data"),
                    {
                      ...
                      gameType: newGame.gameType || "fetan-spin",
                    }
                  );
                  logger.info(
                  ...
                  logger.error(
                    `Error auto-creating new Spin game after completion: ${err.message}`
                  );
                }
              }, 15000);
            }, 15000);
          ...
          logger.error(`Error in join_spin_game: ${error.message}`);
          socket.emit("error", {
    ...
    socket.on("disconnect", () => {
      logger.info(`Spin socket disconnected: ${socket.id}`);
    });
  });
};

module.exports = { initializeSpinSocket };
