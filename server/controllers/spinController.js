...
const createGameRoom = async (req, res) => {
    try {
        const { bet_amount, max_players } = req.body;
        let { prize_tiers, system_benefit } = req.body;
        const gameType = "fetan-spin";
        if (
            !bet_amount ||
            !max_players ||
            isNaN(bet_amount) ||
...
const resetGameRoom = async (req, res) => {
    try {
        ...
        req.app.get("io").emit("fetan_spin_rooms", gamesWithPrizeStructure);
...
const listGameHistory = async (req, res) => {
    try {
        ...
        const query = { gameType: "fetan-spin" };
