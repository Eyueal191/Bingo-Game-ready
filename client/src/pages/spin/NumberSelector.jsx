...
const NumberSelector = ({
  ...
  const handleNumberClick = (number) => {
    if (takenSet.has(number)) {
      setError(`Number ${number} is already taken`);
      return;
    }
    setError("");
    onSelect(number);
    ...
    <Box sx={{ maxWidth: "600px", mx: "auto" }}>
      ...
      >
        {Array.from({ length: maxNumbers }, (_, i) => i + 1).map((number) => {
          ...
          const bg = isTaken ? "#ff3b30" : isSelected ? "#116e51" : "#00ff1f";
          ...
          const boxShadow = isTaken
            ? "0 4px 10px rgba(255, 59, 48, 0.3)"
            : isSelected
              ? "0 4px 12px rgba(17, 110, 81, 0.4)"
              : "0 4px 14px rgba(0, 255, 31, 0.4)";
          const hoverBoxShadow = isTaken
            ? "0 6px 16px rgba(255, 59, 48, 0.4)"
            : isSelected
              ? "0 6px 18px rgba(17, 110, 81, 0.5)"
              : "0 6px 18px rgba(0, 255, 31, 0.5)";
          return (
            <Button
              key={number}
              ...
              sx={{
                "&&": {
                  ...
                  "&.Mui-disabled": {
                    ...
                    boxShadow: "0 4px 10px rgba(255, 59, 48, 0.3) !important",
                  },
                },
              }}
            >
