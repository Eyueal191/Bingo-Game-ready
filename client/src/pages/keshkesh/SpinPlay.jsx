...
const SpinPlay = () => {
  ...
  useEffect(() => {
    ...
    const onUpdate = (data) => {
      ...
      if (data.type === "winner") {
        ...
        setWinners((prev) => {
          ...
          //     resolved.prize
          //       ? ` — ${Number(resolved.prize).toFixed(2)} ብር`
          //       : ""
          //   }!`
          // );
    ...
    socket.on("keshkesh_room_data", onRoom);
    ...
    return () => {
      socket.off("keshkesh_room_data", onRoom);
  ...
  if (!room) return <Typography sx={{ py: 10 }}>የክፍል መረጃ የለም</Typography>;
    ...
    >
      ...
      >
        ...
        <Typography variant="h6" sx={{ color: "#ffd700", fontWeight: 700 }}>
          — መደብ {room.bet_amount} ብር
        </Typography>
        <Typography variant="subtitle1" sx={{ color: "#d0d0d0" }}>
          · ተጫዋቾች: {room.max_players ? room.max_players : 0}
        </Typography>
      </Box>
      ...
      {!isSpinning && !winner && !hasStarted && (
        ...
        >
          <Typography
            sx={{ color: "#e0e0e0", textAlign: "center", fontWeight: 600 }}
          >
            ለማሽከርከር በመዘጋጀት ላይ…
          </Typography>
        </Box>
          ...
          <SpinWheel
            ...
            onSpinComplete={(spinId) => {
              ...
              if (
                ...
                // toast.success(
                //   `🎉 Winner: ${resolved.label}${
                //     resolved.prize ? ` — ${resolved.prize} ብር` : ""
                //   }!`
                // );
          ...
          <AnimatePresence>
            {latestWinner && (
              ...
              >
                ...
                >
                  ደረጃ {latestWinner.rank} አሸናፊ: ቁጥር {latestWinner.number} (
                  {latestWinner.user})
                </Typography>
                <Typography
                  variant="body2"
                ...
                >
                  ሽልማት: {latestWinner.prize.toFixed(2)} ብር
                </Typography>
              </motion.div>
            )}
          </AnimatePresence>
      ...
      {isGameCompleted && (
        ...
        >
          🎉 ጨዋታ ተጠናቋል! ወደ ጨዋታዎች ገፅ በመመለስ ላይ...
        </Alert>
      )}
    </Box>
  );
