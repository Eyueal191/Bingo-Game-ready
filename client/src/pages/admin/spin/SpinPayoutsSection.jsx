...
const SpinPayoutsSection = ({ payouts, setPayouts, setError }) => {
  const api = useApi();
  ...
  const handleUpdatePayout = async (payoutId, status) => {
    try {
      await api.put(`/api/v1/spin/payouts/${payoutId}`, { status });
      setPayouts(
        ...
        <Table sx={tableSx} aria-label="payouts table">
          <TableHead>
            <TableRow>
              ...
              <TableCell>Amount (coins)</TableCell>
...
export default SpinPayoutsSection;
