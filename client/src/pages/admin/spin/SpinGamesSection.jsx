...
const SpinGamesSection = ({ games, setGames }) => {
  const api = useApi();
  ...
  const handleCreateGame = async () => {
    try {
      ...
      const response = await api.post("/api/v1/spin/create", {
        bet_amount,
        max_players,
        system_benefit,
        gameType: createForm.gameType || "spin",
        prize_tiers,
      });
      setGames([...games, response.data]);
      setSnackbar({
        open: true,
        message: "Spin-Spin room created successfully",
        severity: "success",
      });
      toast.success("Spin-Spin room created successfully");
      handleCloseDialog();
    } catch (err) {
      const errorMessage =
        err.response?.data?.error ||
        err.message ||
        "Failed to create Spin-Spin room";
  ...
  const handleUpdateGame = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: "Spin-Spin room updated successfully",
        severity: "success",
      });
      toast.success("Spin-Spin room updated successfully");
      handleCloseDialog();
    } catch (err) {
      const errorMessage =
        err.response?.data?.error ||
        err.message ||
        "Failed to update Spin-Spin room";
      setSnackbar({ open: true, message: errorMessage, severity: "error" });
      toast.error(errorMessage);
    }
  };
  ...
  const handleResetGame = async (gameId) => {
    try {
      await api.post(`/api/v1/spin/${gameId}/reset`);
  ...
  const handleDeleteGame = async (gameId) => {
    try {
      await api.delete(`/api/v1/spin/${gameId}`);
  ...
  const tableSx = {
    ...
    "& th": {
      ...
      background: "linear-gradient(145deg, #116e51, #116e51)",
      color: "#ffffff",
      textAlign: "center",
      whiteSpace: "nowrap",
      boxShadow:
    ...
    "& tr": {
      background: "linear-gradient(145deg, #116e51, #0f1221)",
      boxShadow:
    ...
    "& tr:nth-of-type(even)": {
      background: "linear-gradient(145deg, #0f1221, #0f1221)",
    },
  };
  ...
  return (
    ...
    >
      ...
      >
        {`Create New ${
          createForm.gameType === "fetan-spin" ? "Fetan-Spin" : "Spin-Spin"
        } Room`}
      </Button>
      <TableContainer
        component={Paper}
        ...
        <Table sx={tableSx} aria-label="games table">
          <TableHead>
            <TableRow>
              ...
              <TableCell>Prize Amount (coins)</TableCell>
              <TableCell>Bet Amount (coins)</TableCell>
          ...
          {dialog.action === "create" ? (
            <>
              ...
              <FormControl fullWidth margin="normal">
                ...
                >
                  <MenuItem value="spin">Spin-Spin</MenuItem>
...
export default SpinGamesSection;
